...
object ExportHelper {
    fun exportCsv(c: Context, items: List<Product>, affId: String = ""): File {
        ...
        FileWriter(f).use { fw ->
            ...
            items.forEachIndexed { i, it ->
                p.printRecord(i+1,it.name,it.price,it.sold,"%.1f".format(it.soldPerDay),it.rating,it.commissionEst,it.velocityBadge?:"",AffiliateLink.toAffiliate(it.link, affId),it.shopName,it.flagReason?:"")
            }
            p.flush()
        }
        return f
